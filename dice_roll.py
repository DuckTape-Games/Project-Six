...
def turn(player_dice,enemy_dice):
    ...
    player_turn_total = (0,None,False) #3 values are turn total, rolled effect (when applicable), and whether or not duct tape is active
    enemy_turn_total = (0,None,False) #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    for i in range(len(player_dice)):
        ...
        if player_turn_total[1] != "right":
            ...
            player_turn_total = (turn[0], turn[1], turn[3])
            enemy_turn_total = (turn[2], enemy_turn_total[1], enemy_turn_total[2])
        else:
            player_turn_total = (player_turn_total[0],None,player_turn_total[2])
        ...
        if enemy_turn_total[1] != "right":
            ...
            enemy_turn_total = (turn[0], turn[1], turn[3])
            player_turn_total = (turn[2], player_turn_total[1], player_turn_total[2])
        else:
            enemy_turn_total = (enemy_turn_total[0],None, enemy_turn_total[2])
...
def check_effect(roll, turn_total, dice, active_die_num, opponent_total, duct_tape):
    global poison_venom

    rolled_num = roll[0]
    if poison_venom is False: #Applies venom
        rolled_num /= 2
        if rolled_num > int(rolled_num):
            rolled_num += 1
        rolled_num = int(rolled_num)
    

    if duct_tape: #Makes the number concatenate instead of adding them normally
        turn_total *= 10 ** len(str(rolled_num))

    turn_total += rolled_num #The rolled number gets added first before the effect is calculated
    ...
    try:
        ...
        if roll[1] == None: #Returns if there is no effect
            return (turn_total,None,opponent_total,duct_tape)
        ...
        if roll[1].lower() == "duct": #Activates the duct tape effect for the rest of the turn
            return (turn_total,None,opponent_total,True)
        
        if roll[1][0] == "+": #Adds the value indecated beside the +
            return (turn_total + effect_value, None, opponent_total,duct_tape)
        ...
        if roll[1][0] in ["-","/","v","V"]:
            if opponent_total > 0:
                ...
                if roll[1][0] == "-": #Subtracts from the opponent's total
                    ...
                    if opponent_total <= 0:
                        return (turn_total, None, 0, duct_tape)
                    return (turn_total, None, opponent_total,duct_tape)
        ...
        if roll[1].lower() == "left": #Rerolls both the previous die and the active die. left acts as center when it is the first die in the sequence
            if active_die_num > 0:
                ...
                turn_total = (turn_total,None,opponent_total, duct_tape)
        ...
        if roll[1].lower() == "right": #Skips the player's next roll
            return (turn_total,"right",opponent_total,duct_tape)
